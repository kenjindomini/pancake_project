...
function getSortedUsers (range, sortDirection, sortColumn) {
  var p = new Promise(function (resolve, reject) {
    ...
    _.some(range, function (_range) {
      ...
      rangeLimit = (_range.to - _range.from) + 1;
    });
    ...
    // Debug logging
    console.log('getSortedUsers: range.to:' + range.to + ' range.from:' + range.from);
    // End
...
// This will replace getSortedUsers when the call route is fixed.
/*
...
var UsersRouter = Router.createClass([
  {
    ...
    call: function (pathSet, args) {
      ...
      // Debug logging
      console.log('call(sortedUsers): args[0]:' + args[0] + ' args[1]:' + args[1]);
      // End
      var indexOffset = 0;
      ...
      // Debug loggin
      console.log('call(sortedUsers): indexOffset:' + indexOffset);
      // End
  ...
  },
  {
    ...
    get: function (pathSet) {
      ...
      // Debug logging
      console.log('get(usersAscendingSort): indexOffset:' + indexOffset);
      // End
      return getSortedUsers(pathSet.indexRanges, 'asc').then(function (res) {
        // Debug logging
        console.log('usersAscendingSort: number of rows returned by getSortedUsers:' + res.docs.length);
        //End
        var jsonGraph = _(res.docs)
            .map(function (user, i) {
              var index = i + indexOffset;
